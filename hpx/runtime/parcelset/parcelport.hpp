...
namespace hpx { namespace parcelset
{
    ...
    public:
        ...
        // number of messages sent
        std::int64_t get_action_message_send_count(
            std::string const&, bool reset);
        ...
        // number of messages received
        std::int64_t get_action_message_receive_count(
            std::string const&, bool reset);

        // the total time it took for all sends, from async_write to the
        // completion handler (nanoseconds)
        std::int64_t get_action_sending_time(
            std::string const&, bool reset);

        // the total time it took for all receives, from async_read to the
        // completion handler (nanoseconds)
        std::int64_t get_action_receiving_time(
            std::string const&, bool reset);
        ...
        // the total time it took for all sender-side security operations
        // (nanoseconds)
        std::int64_t get_action_sending_security_time(
            std::string const&, bool reset);

        // the total time it took for all receiver-side security
        // operations (nanoseconds)
        std::int64_t get_action_receiving_security_time(
            std::string const&, bool reset);
#endif
        ...
        // total data (uncompressed) sent (bytes)
        std::int64_t get_action_raw_data_sent(
            std::string const&, bool reset);
        ...
        // total data (uncompressed) received (bytes)
        std::int64_t get_action_raw_data_received(
            std::string const&, bool reset);

        std::int64_t get_action_buffer_allocate_time_sent(
            std::string const&, bool reset);

        std::int64_t get_action_buffer_allocate_time_received(
            std::string const&, bool reset);
#endif
        ...
        void add_received_data(std::string const& action,
            performance_counters::parcels::data_point const& data);

        void add_sent_data(std::string const& action,
            performance_counters::parcels::data_point const& data);
