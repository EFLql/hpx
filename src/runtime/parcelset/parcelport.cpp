...
namespace hpx { namespace parcelset
{
    ...
    void parcelport::add_received_data(std::string const& action,
    ...
    void parcelport::add_sent_data(std::string const& action,
        performance_counters::parcels::data_point const& data)
    {
    ...
    // number of messages sent
    std::int64_t parcelport::get_action_message_send_count(
        std::string const& action, bool reset)
    {
        return action_parcels_sent_.num_messages(action, reset);
    }
    ...
    // number of messages received
    std::int64_t parcelport::get_action_message_receive_count(
        std::string const& action, bool reset)
    {
        return action_parcels_received_.num_messages(action, reset);
    }

    // the total time it took for all sends, from async_write to the
    // completion handler (nanoseconds)
    std::int64_t parcelport::get_action_sending_time(
        std::string const& action, bool reset)
    {
        return action_parcels_sent_.total_time(action, reset);
    }

    // the total time it took for all receives, from async_read to the
    // completion handler (nanoseconds)
    std::int64_t parcelport::get_action_receiving_time(
        std::string const& action, bool reset)
    {
        return action_parcels_received_.total_time(action, reset);
    }
    ...
    // the total time it took for all sender-side security operations
    // (nanoseconds)
    std::int64_t parcelport::get_action_sending_security_time(
        std::string const& action, bool reset)
    {
        return action_parcels_sent_.total_security_time(action, reset);
    }

    // the total time it took for all receiver-side security
    // operations (nanoseconds)
    std::int64_t parcelport::get_action_receiving_security_time(
        std::string const& action, bool reset)
    {
        return action_parcels_received_.total_security_time(action, reset);
    }
#endif
    ...
    // total data (uncompressed) sent (bytes)
    std::int64_t parcelport::get_action_raw_data_sent(
        std::string const& action, bool reset)
    {
        return action_parcels_sent_.total_raw_bytes(action, reset);
    }
    ...
    // total data (uncompressed) received (bytes)
    std::int64_t parcelport::get_action_raw_data_received(
        std::string const& action, bool reset)
    {
        return action_parcels_received_.total_raw_bytes(action, reset);
    }

    std::int64_t parcelport::get_action_buffer_allocate_time_sent(
        std::string const& action, bool reset)
    {
        return action_parcels_sent_.total_buffer_allocate_time(action, reset);
    }

    std::int64_t parcelport::get_action_buffer_allocate_time_received(
        std::string const& action, bool reset)
    {
        return action_parcels_received_.total_buffer_allocate_time(action, reset);
    }
#endif
