...
//             if (!resolver_->has_resolved_locality(locality))
    ...
    // number of messages sent
    std::int64_t parcelhandler::get_action_message_send_count(
        std::string const& pp_type, std::string const& action, bool reset) const
    {
        error_code ec(lightweight);
        parcelport* pp = find_parcelport(pp_type, ec);
        return pp ? pp->get_action_message_send_count(action, reset) : 0;
    }
    ...
    // number of messages received
    std::int64_t parcelhandler::get_action_message_receive_count(
        std::string const& pp_type, std::string const& action, bool reset) const
    {
        error_code ec(lightweight);
        parcelport* pp = find_parcelport(pp_type, ec);
        return pp ? pp->get_action_message_receive_count(action, reset) : 0;
    }

    // the total time it took for all sends, from async_write to the
    // completion handler (nanoseconds)
    std::int64_t parcelhandler::get_action_sending_time(
        std::string const& pp_type, std::string const& action, bool reset) const
    {
        error_code ec(lightweight);
        parcelport* pp = find_parcelport(pp_type, ec);
        return pp ? pp->get_action_sending_time(action, reset) : 0;
    }

    // the total time it took for all receives, from async_read to the
    // completion handler (nanoseconds)
    std::int64_t parcelhandler::get_action_receiving_time(
        std::string const& pp_type, std::string const& action, bool reset) const
    {
        error_code ec(lightweight);
        parcelport* pp = find_parcelport(pp_type, ec);
        return pp ? pp->get_action_receiving_time(action, reset) : 0;
    }
    ...
    // the total time it took for all sender-side security operations
    // (nanoseconds)
    std::int64_t parcelhandler::get_action_sending_security_time(
        std::string const& pp_type, std::string const& action, bool reset) const
    {
        error_code ec(lightweight);
        parcelport* pp = find_parcelport(pp_type, ec);
        return pp ? pp->get_action_sending_security_time(action, reset) : 0;
    }

    // the total time it took for all receiver-side security
    // operations (nanoseconds)
    std::int64_t parcelhandler::get_action_receiving_security_time(
        std::string const& pp_type, std::string const& action, bool reset) const
    {
        error_code ec(lightweight);
        parcelport* pp = find_parcelport(pp_type, ec);
        return pp ? pp->get_action_receiving_security_time(action, reset) : 0;
    }
#endif
    ...
    // total data (uncompressed) sent (bytes)
    std::int64_t parcelhandler::get_action_raw_data_sent(
        std::string const& pp_type, std::string const& action, bool reset) const
    {
        error_code ec(lightweight);
        parcelport* pp = find_parcelport(pp_type, ec);
        return pp ? pp->get_action_raw_data_sent(action, reset) : 0;
    }
    ...
    // total data (uncompressed) received (bytes)
    std::int64_t parcelhandler::get_action_raw_data_received(
        std::string const& pp_type, std::string const& action, bool reset) const
    {
        error_code ec(lightweight);
        parcelport* pp = find_parcelport(pp_type, ec);
        return pp ? pp->get_action_raw_data_received(action, reset) : 0;
    }

    std::int64_t parcelhandler::get_action_buffer_allocate_time_sent(
        std::string const& pp_type, std::string const& action, bool reset) const
    {
        error_code ec(lightweight);
        parcelport* pp = find_parcelport(pp_type, ec);
        return pp ? pp->get_action_buffer_allocate_time_sent(action, reset) : 0;
    }

    std::int64_t parcelhandler::get_action_buffer_allocate_time_received(
        std::string const& pp_type, std::string const& action, bool reset) const
    {
        error_code ec(lightweight);
        parcelport* pp = find_parcelport(pp_type, ec);
        return pp ? pp->get_action_buffer_allocate_time_received(action, reset) : 0;
    }
#endif
    ...
    void parcelhandler::register_counter_types(std::string const& pp_type)
    {
        ...
        util::function_nonser<std::int64_t(bool)> sending_serialization_time(
            util::bind(&parcelhandler::get_sending_serialization_time, this,
                pp_type, _1));
        util::function_nonser<std::int64_t(bool)> receiving_serialization_time(
            util::bind(&parcelhandler::get_receiving_serialization_time, this,
                pp_type, _1));
        ...
        util::function_nonser<std::int64_t(bool)> data_sent(
            util::bind(&parcelhandler::get_data_sent, this,
                pp_type, _1));
        util::function_nonser<std::int64_t(bool)> data_received(
            util::bind(&parcelhandler::get_data_received, this,
                pp_type, _1));
